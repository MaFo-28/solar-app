/**
 * tab-simulation.js
 * Onglet Simulation — bac à sable à 3 journées (été/équinoxe/hiver)
 * pour visualiser le fonctionnement du système : on choisit un profil
 * d'installation (défini dans l'onglet Installation) et un profil de
 * consommation (onglet Consommation), et on obtient la courbe de
 * production théorique + les résultats pas-à-pas (charge/décharge
 * batterie, import/export réseau, coûts).
 *
 * Ne gère ni le matériel ni les stratégies (ça, c'est l'onglet
 * Installation) — cet onglet ne fait que choisir un profil déjà
 * défini et en visualiser le fonctionnement.
 */
window.TabSimulation = (function () {
  "use strict";

  const charts = {}; // clé "saison" -> instance Chart.js (résultats)
  let pendingCharts = [];
  ...
  function renderInstallSelect() {
    ...
    const current = s.simulationConfig.installationProfileId;
  ...
  function renderConsumptionSelect() {
    ...
    const s = window.AppState.get();
    const current = s.simulationConfig.consumptionProfileId;
    if (current && profiles.some((p) => p.id === current)) {
      select.value = current;
    } else if (profiles.length > 0) {
      select.value = profiles[0].id;
  ...
  // ------------------------------------------------------------------
  // Rafraîchissement croisé (profils d'installation, bases, localisation, consommation)
  // ------------------------------------------------------------------
  ...
  function buildContext() {
    ...
    const panel = installProfile
      ? (s.panelsDatabase || []).find((p) => p.id === installProfile.panels.selectedModelId)
      : null;
    const inverter = installProfile
      ? (s.invertersDatabase || []).find((i) => i.id === installProfile.inverter.selectedModelId)
      : null;
    const battery = installProfile
      ? (s.batteriesDatabase || []).find((b) => b.id === installProfile.battery.selectedModelId)
      : null;
    const consumptionProfile = (s.consumption.profiles || []).find((p) => p.id === sim.consumptionProfileId) || null;
  ...
  function validate(ctx) {
    const errors = [];
    if (!ctx.installProfile) errors.push("Sélectionnez un profil d'installation (onglet Installation).");
    if (ctx.installProfile && !ctx.panel) errors.push("Le profil d'installation choisi n'a pas de panneau sélectionné.");
    ...
    if (!ctx.battery && !ctx.inverter && ctx.installProfile) {
  ...
  function recompute() {
    ...
    resultsEl.appendChild(renderProfileResults(ctx));
    flushPendingCharts();
  }
  ...
  function renderPowerChartSection(ctx) {
    ...
    const mask = ctx.installProfile.panels.horizonMaskProfileId
      ? ((loc.horizonMaskProfiles.find((p) => p.id === ctx.installProfile.panels.horizonMaskProfileId) || {}).mask || [])
      : [];

    renderPowerChart(loc, ctx.installProfile.panels, ratedTotalWc, mask);
  }
  ...
  // ------------------------------------------------------------------
  // Résultats (simulation pas-à-pas, 3 saisons)
  // ------------------------------------------------------------------
  const SEASONS = [
  ...
  function runSimulationForSeason(ctx, seasonKey) {
    ...
    const tzMap = { ete: 2, hiver: 1, equinoxe: 2 };
    ...
    const powerCurve = PV.dayPowerCurve(
      ...
      ratedTotalWc, install.panels.tilt, install.panels.orientation, mask, 10
    );
  ...
  function renderProfileResults(ctx) {
